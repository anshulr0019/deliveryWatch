...
export function LoginForm() {
  ...
  return (
    <SpotlightCard className="w-full max-w-md p-1" borderGlowColor="rgba(200, 169, 110, 0.5)">
      <div className="rounded-[14px] bg-[#0B0D12]/85 p-7 sm:p-9">
          ...
          {(["signin", "signup"] as Mode[]).map((m) => (
            <button
              ...
              className={`rounded-lg px-3 py-2 font-medium transition ${
                mode === m ? "bg-gradient-to-br from-gold-light via-gold to-gold-deep text-obsidian shadow" : "text-muted hover:text-white"
              }`}
            >
        ...
        <p className="mt-5 text-center text-xs text-muted-2">
          {mode === "signin" ? (
            <>
              New here?{" "}
              <button type="button" onClick={() => setMode("signup")} className="text-gold hover:text-gold-light">
            ...
            <>
              ...
              <button type="button" onClick={() => setMode("signin")} className="text-gold hover:text-gold-light">
                Sign in
              </button>
            </>
          )}
